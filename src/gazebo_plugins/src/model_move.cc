...
namespace gazebo
{
  ...
  GazeboRosModelMove::GazeboRosModelMove() {}

  GazeboRosModelMove::~GazeboRosModelMove() {}

  // Load the controller
  void GazeboRosModelMove::Load(physics::ModelPtr parent,
      sdf::ElementPtr sdf)
  {
    ...
    {
      ROS_INFO_NAMED("model_move", "modelMovePlugin missing <robotNamespace>, "
    ...
    {
      ROS_INFO_NAMED("model_move", "modelMovePlugin missing <enableYAxis>, "
    ...
    {
      ROS_WARN_NAMED("model_move", "modelMovePlugin (ns = %s) missing <commandTopic>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("model_move", "modelMovePlugin (ns = %s) missing <odometryTopic>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("model_move", "modelMovePlugin (ns = %s) missing <odometryFrame>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("model_move", "modelMovePlugin (ns = %s) missing <robotBaseFrame>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("model_move", "modelMovePlugin (ns = %s) missing <odometryRate>, "
          "defaults to %f",
    ...
    {
      ROS_FATAL_STREAM_NAMED("model_move", "modelMovePlugin (ns = " << robot_namespace_
    ...
    ROS_DEBUG_NAMED("model_move", "OCPlugin (%s) has started",
    ...
    ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
          boost::bind(&GazeboRosModelMove::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);
    ...
    callback_queue_thread_ =
      boost::thread(boost::bind(&GazeboRosModelMove::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ =
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&GazeboRosModelMove::UpdateChild, this));

  }

  // Update the controller
  void GazeboRosModelMove::UpdateChild()
  {
  ...
  void GazeboRosModelMove::FiniChild() {
    alive_ = false;
    queue_.clear();
    queue_.disable();
    rosnode_->shutdown();
  ...
  void GazeboRosModelMove::cmdVelCallback(
  ...
  void GazeboRosModelMove::QueueThread()
  {
  ...
  void GazeboRosModelMove::publishOdometry(double step_time)
  {
  ...
  GZ_REGISTER_MODEL_PLUGIN(GazeboRosModelMove)
}
