...
void UnderwaterObjectROSPlugin::Load(gazebo::physics::ModelPtr _parent,
  ...
  //-add namespace
  //-remove topic prefix + "/"
  myRosNode = gazebo_ros::Node::CreateWithArgs(
      _sdf->Get<std::string>("name")); //, _parent->GetName());
  gzmsg << "[UnderwaterObjectROSPlugin]: Node created. Name: "
        << myRosNode->get_name()
  ...
  this->services["set_added_mass_scaling"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::SetFloat>(
          ...
          std::bind(&UnderwaterObjectROSPlugin::SetScalingAddedMass, this, _1,
                    _2));
  ...
  this->services["get_added_mass_scaling"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::GetFloat>(
          ...
          std::bind(&UnderwaterObjectROSPlugin::GetScalingAddedMass, this, _1,
                    _2));

  this->services["set_damping_scaling"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::SetFloat>(
          _parent->GetName() + "/set_damping_scaling",
          std::bind(&UnderwaterObjectROSPlugin::SetScalingDamping, this, _1,
                    _2));

  this->services["get_damping_scaling"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::GetFloat>(
          _parent->GetName() + "/get_damping_scaling",
          std::bind(&UnderwaterObjectROSPlugin::GetScalingDamping, this, _1,
                    _2));

  this->services["set_volume_scaling"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::SetFloat>(
          _parent->GetName() + "/set_volume_scaling",
          std::bind(&UnderwaterObjectROSPlugin::SetScalingVolume, this, _1,
                    _2));

  this->services["get_volume_scaling"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::GetFloat>(
          _parent->GetName() + "/get_volume_scaling",
          std::bind(&UnderwaterObjectROSPlugin::GetScalingVolume, this, _1,
                    _2));
  ...
  this->services["set_added_mass_offset"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::SetFloat>(
          ...
          std::bind(&UnderwaterObjectROSPlugin::SetOffsetAddedMass, this, _1,
                    _2));

  this->services["get_added_mass_offset"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::GetFloat>(
          _parent->GetName() + "/get_added_mass_offset",
          std::bind(&UnderwaterObjectROSPlugin::GetOffsetAddedMass, this, _1,
                    _2));

  this->services["set_linear_damping_offset"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::SetFloat>(
          _parent->GetName() + "/set_linear_damping_offset",
          std::bind(&UnderwaterObjectROSPlugin::SetOffsetLinearDamping, this,
                    _1, _2));

  this->services["get_linear_damping_offset"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::GetFloat>(
          _parent->GetName() + "/get_linear_damping_offset",
          std::bind(&UnderwaterObjectROSPlugin::GetOffsetLinearDamping, this,
                    _1, _2));
  ...
  this->services["set_nonlinear_damping_offset"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::SetFloat>(
          ...
          std::bind(&UnderwaterObjectROSPlugin::SetOffsetNonLinearDamping, this,
                    _1, _2));

  this->services["get_nonlinear_damping_offset"] =
      myRosNode->create_service<uuv_gazebo_ros_plugins_msgs::srv::GetFloat>(
          _parent->GetName() + "/get_nonlinear_damping_offset",
          std::bind(&UnderwaterObjectROSPlugin::GetOffsetNonLinearDamping, this,
                    _1, _2));

  this->services["get_model_properties"] = myRosNode->create_service<
...
void UnderwaterObjectROSPlugin::InitDebug(gazebo::physics::LinkPtr _link,
  ...
  for (std::map<std::string, gazebo::transport::PublisherPtr>::iterator it =
           this->hydroPub.begin();
...
void UnderwaterObjectROSPlugin::PublishRestoringForce(
  ...
  if (this->models.count(_link)) {
    ...
    ignition::math::Vector3d restoring =
        this->models[_link]->GetStoredVector(RESTORING_FORCE);
    ...
    auto tmpPub = std::static_pointer_cast<
        ...
        myRosHydroPub[_link->GetName() + "/restoring"]);
    tmpPub->publish(msg);
  }
}
...
void UnderwaterObjectROSPlugin::PublishIsSubmerged() {
  ...
  isSubmerged.data =
      this->models[this->model->GetLink(this->baseLinkName)]->IsSubmerged();

  auto tmpPub =
      std::static_pointer_cast<rclcpp::Publisher<std_msgs::msg::Bool>>(
          myRosHydroPub["is_submerged"]);
...
void UnderwaterObjectROSPlugin::PublishCurrentVelocityMarker() {
  ...
  std::static_pointer_cast<rclcpp::Publisher<visualization_msgs::msg::Marker>>(
      myRosHydroPub["current_velocity_marker"])
      ->publish(marker);
...
void UnderwaterObjectROSPlugin::PublishHydrodynamicWrenches(
  ...
  if (this->models.count(_link)) {
    ...
    std::static_pointer_cast<
        rclcpp::Publisher<geometry_msgs::msg::WrenchStamped>>(
        myRosHydroPub[_link->GetName() + "/added_mass"])
        ->publish(msg);
    ...
    std::static_pointer_cast<
        rclcpp::Publisher<geometry_msgs::msg::WrenchStamped>>(
        myRosHydroPub[_link->GetName() + "/damping"])
        ->publish(msg);
    ...
    std::static_pointer_cast<
        rclcpp::Publisher<geometry_msgs::msg::WrenchStamped>>(
        myRosHydroPub[_link->GetName() + "/added_coriolis"])
        ->publish(msg);
...
#if GAZEBO_MAJOR_VERSION >= 8
  _output.header.stamp =
      rclcpp::Time(this->world->SimTime().sec,
                   this->world->SimTime()
                       .nsec); // ros::Time(this->world->SimTime().Double());
#else
  _output.header.stamp =
      rclcpp::Time(this->world->SimTime().sec,
                   this->world->SimTime()
                       .nsec); // ros::Time(this->world->GetSimTime().Double());
#endif
}
...
void UnderwaterObjectROSPlugin::SetUseGlobalCurrentVel(
    const uuv_gazebo_ros_plugins_msgs::srv::SetUseGlobalCurrentVel::Request::
        SharedPtr _req,
  ...
  if (_req->use_global == this->useGlobalCurrent)
    ...
    if (this->useGlobalCurrent)
      gzmsg << this->model->GetName() << "::Now using global current velocity"
            << std::endl;
    else
...
void UnderwaterObjectROSPlugin::GetModelProperties(
    const uuv_gazebo_ros_plugins_msgs::srv::GetModelProperties::Request::
        SharedPtr /*_req*/,
    uuv_gazebo_ros_plugins_msgs::srv::GetModelProperties::Response::SharedPtr
        _res) {
...
void UnderwaterObjectROSPlugin::GetScalingDamping(
    const uuv_gazebo_ros_plugins_msgs::srv::GetFloat::Request::
        SharedPtr /*_req*/,
    uuv_gazebo_ros_plugins_msgs::srv::GetFloat::Response::SharedPtr _res)
...
{
  models.begin()->second->GetParam("offset_lin_forward_speed_damping",
                                   _res->data);
}
